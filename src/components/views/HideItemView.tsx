...
export default function HideItemView() {
  ...
  const [formData, setFormData] = useState({
    agent_name: '', reason: '', action_to_unhide: '', comment: '', responsible_party: ''
  });
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === 'PRODUCT_CREATED' || data.type === 'PRODUCT_UPDATED' || data.type === 'PRODUCT_DELETED' || data.type === 'HIDDEN_ITEMS_UPDATED') {
        fetchInitialData();
    ...
    try {
      ...
      if (branchesRes.ok) {
        ...
        if (Array.isArray(allBranches)) {
          const filteredBranches = allBranches.filter((b: Branch) => b.brand_id === Number(selectedBrand));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          setSuccess(false);
          setFormData({ agent_name: '', reason: '', action_to_unhide: '', comment: '', responsible_party: '' });
        ...
        >
          <div className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 p-8 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.05)] relative overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <input 
                      ...
                      className="w-full bg-zinc-50 dark:bg-zinc-950 border-2 border-zinc-100 dark:border-zinc-800 rounded-xl pl-12 pr-4 py-3.5 text-sm font-bold text-zinc-900 dark:text-white focus:border-zinc-900 dark:focus:border-white transition-all outline-none shadow-sm"
                    />
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <input 
                      type="text"
                      placeholder="e.g., Supply Chain Delay"
                      required
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 dark:text-zinc-500 ml-1">Restoration Action</label>
                <div className="relative group">
                  ...
                  <input 
                    type="text"
                    placeholder="What steps are needed to unhide this item?"
                    value={formData.action_to_unhide}
                    onChange={(e) => setFormData({...formData, action_to_unhide: e.target.value})}
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border-2 border-zinc-100 dark:border-zinc-800 rounded-xl pl-12 pr-4 py-3.5 text-sm font-bold text-zinc-900 dark:text-white focus:border-zinc-900 dark:focus:border-white transition-all outline-none shadow-sm"
                  />
                </div>
              </div>
